...
- **Qwen2.5-VL GGUF 支持**（可选）— 通过 llama.cpp 运行 Qwen2.5-VL-7B/72B，支持图片和**视频分析**
...
| Qwen2.5-VL-72B (GGUF) | >=20GB | 是 | >=16GB |
...
- **Qwen2.5-VL-72B**: 需要 2 个文件 — 主模型 GGUF + mmproj GGUF（~45GB）
...
| `scripts/qwen_analyze.py` | Qwen2.5-VL-7B/72B GGUF 图片 + 视频分析 |
